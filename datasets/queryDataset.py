...
"""
# updated code with scaleimage class
"""
import random
from PIL import Image
import numpy as np
...
class ScaleImage(object):
    def __init__(self, scale_range=(-100, 100)):
        self.scale_range = scale_range

    def __call__(self, img):
        scale_factor = random.uniform(self.scale_range[0], self.scale_range[1])
        scale_factor = (scale_factor + 100) / 100  # Convert to scale range (0 to 2)
...
if __name__ == '__main__':
    ...
    target_dir = "/home/ming.dai/workspace/code/DenseUAV/visualization/scale_image"
    os.makedirs(target_dir, exist_ok=True)
    for ind in range(10):
        image_ = si(image)
        image_ = np.array(image_)
