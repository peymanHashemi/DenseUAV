...
class RandomErasing(object):
    ...
    def __init__(self, probability=0.5, sl=0.05, sh=0.5, r1=0.5):
...
class RandomScale(object):
    def __init__(self, scale_range=(-50,50)):
        self.scale_range = scale_range

    def __call__(self, img):
        scale_factor = random.uniform(self.scale_range[0], self.scale_range[1])
        
        scale_factor = 1 + scale_factor / 100 
        ...
        # Center crop or pad the image to original size
        if scale_factor > 1:  # Zoom in, crop to the original size
            left = (new_width - width) // 2
            top = (new_height - height) // 2
            right = (new_width + width) // 2
            bottom = (new_height + height) // 2
            image = img.resize((new_width, new_height), Image.ANTIALIAS).crop((left, top, right, bottom))
        else:  # Zoom out, pad to the original size
...
if __name__ == '__main__':
    ...
    ra = transforms.RandomAffine(360)
